...
def request_api_data(hash_head):
    url = 'https://api.pwnedpasswords.com/range/' + hash_head # <-what you can and can't scrape
...
def password_count(respond, hash_tail):
    ...
    respond = (line.split(':') for line in respond.text.splitlines()) # <- tuple. Using splitline to split eachline, not each character.
